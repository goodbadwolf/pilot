...
namespace mpi
{
Messenger::Messenger(std::shared_ptr<beams::mpi::MpiEnv> mpiEnv)
  : Mpi(mpiEnv)
{
}
                              ...
                              std::vector<RequestTagPair>& reqTags)
{
  ...
  if (err != MPI_SUCCESS)
    throw vtkm::cont::ErrorBadValue("Error with MPI_Testsome in Messenger::RecvData");
...
void Messenger::SendData(int dst, int tag, const vtkmdiy::MemoryBuffer& buff)
{
  ...
  for (const auto& b : bufferList)
  {
    ...
    if (err != MPI_SUCCESS)
      throw vtkm::cont::ErrorBadValue("Error in MPI_Isend inside Messenger::SendData");
                         ...
                         bool blockAndWait)
{
  ...
  for (const auto& rt : reqTags)
  {
    ...
    if (it == this->RecvBuffers.end())
      throw vtkm::cont::ErrorBadValue("receive buffer not found");
